  ...
  - 예제4. 회원권한을 id옆에 붙여서 표시하는 방법. 그대로 응용하면 권한에 따라 메뉴 보이게 지정
    ...
    # EL (Expression Language. 표현언어)
      ...
      - 중요
        ...
        - JSP의 4개Scope(page, request, session, application)에 존재하는 데이터**만** EL표현식으로 사용가능하다.
          - 즉 ${}안에는 자바단의 변수를 했다고 해서 표현할수 있는 것이 아니다.
...
- Redirect
  ...
  - request, response 객체가 새롭게 생성된다. 이전에 요청했던 정보는 사라진다.
- Forward
  - 자기가 다른 상담원에게 물어봐서 답을 얻고 처리해주는방식. url안변한다.
  - request, response 객체를 공유한다. 즉, 내상황(같은말) 또 안 전해줘도된다.
- 사용법 예
  게시판 글쓰기를 수행할때 둘중 뭘 사용해야할까?
  => redirect.
  사용자가 실수로 혹은 고의로 새로고침을 하면 요청정보가 살아있기 때문에 같은 제목과 내용의 글이 여러번 등록 될 수 있다.
  => 결론
